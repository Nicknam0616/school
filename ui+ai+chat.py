...
class CameraThread(QThread):
    ...
    def face_tracking(self, frame):
        ...
        return cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # RGB로 변환하여 반환
...
class WindowClass(QMainWindow, form_class):
    def __init__(self):
        ...
        self.autonomous_mode = False  # 기본값으로 자율주행 꺼짐
        ...
        # 속도 관련 QTextEdit 설정
        self.speed_set.setReadOnly(True)  # 속도 설정
        self.speed_real.setReadOnly(True)  # 실제 속도
        self.coordinate.setReadOnly(True)  # GPS 좌표

        # 카메라 스레드 시작
        self.camera_thread = CameraThread("basic")  # 기본 모드로 시작
        self.camera_thread.change_pixmap_signal.connect(self.update_image)
        self.camera_thread.update_line_signal.connect(self.update_line)
        self.camera_thread.start()

        # 기본 시작 시 "반자율 주행 OFF" 메시지 출력
        ...
        self.b_r_2.clicked.connect(self.toggle_autonomous_mode)  # 자율주행 on/off 버튼
    ...
    def receive_message(self, data):
        self.chat.append(f"{data['user']}: {data['msg']}")  # 발신자 이름을 포함함
    ...
    def update_speed_set(self):
        ...
        self.speed_set.setText(f"Speed Set: {speed_value}")  # 슬라이더 값에 따라 속도 설정 업데이트

    def update_real_speed(self, real_speed):
        self.speed_real.setText(f"Actual Speed: {real_speed}")  # 아두이노로부터 받아온 실제 속도 업데이트

    def update_coordinate(self, latitude, longitude):
        self.coordinate.setText(f"Coordinate: {latitude}, {longitude}")  # GPS 좌표 업데이트

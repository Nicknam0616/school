...
layout = [
    ...
    [sg.Image(filename='', key='result_image')]
]

# 창 설정
window = sg.Window('RC 탱크카 제어', layout, location=(800, 400), background_color='black', return_keyboard_events=True)
...
while True:
    ...
    if ser.in_waiting > 0:
        ...
        print(serial_data)  # 시리얼 데이터를 출력합니다.
        if serial_data == '1':
            window['fire_status'].update('1')
        elif serial_data == '2':
            window['fire_status'].update('2')
    ...
    if keyboard.is_pressed('w'):
        ser.write(b'w')  # 'w' 키를 누르면 아두이노로 'w' 신호를 보냅니다.
        window['way'].update('w')  # 'w' 키를 누르면 way 값에 'w'를 표시합니다.
    if keyboard.is_pressed('s'):
        ser.write(b's')  # 's' 키를 누르면 아두이노로 's' 신호를 보냅니다.
